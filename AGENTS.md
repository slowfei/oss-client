...
2. `go mod init`: root module initialized as `github.com/maqian/oss-client` with `go 1.22` (the originally planned floor). Root `go.sum` is empty (stdlib-only); third-party transitive cost is contained inside the hoisted `pkg/testkit/contract` module — see item 6 — done.
