...
4. `s3common` extraction: deferred to M3+ once two S3-family drivers have shipped. **M2 lessons (post-aws + minio ship)**: candidate extractions identified — pointer-flatten helpers, range header formatter (`bytes=START-END`), metadata case-folding (both directions), DeleteMany 1000-key batching threshold, S3-compat error-code → `pkg/uos.Code` table. Wait for at least one M3 driver (Alibaba/Tencent/Huawei/Volcengine) to confirm the duplication, then extract — see `RELEASING.md` §5 Follow-up #4.
