...
2. `go mod init`: root module initialized as `github.com/slowfei/oss-client` with `go 1.22` (the originally planned floor). Root `go.sum` is empty (stdlib-only); third-party transitive cost is contained inside the hoisted `pkg/testkit/contract` module — see item 6 — done.
...
8. **M5 abstraction validation (qiniu + upyun) — DONE 2026-04-28**: **10 v1-target drivers shipped with zero `pkg/uos` change** — the v1 abstraction is fully validated. All 4 frozen `DirectGrantMode` values exercised in production code: `URL` (S3-family read presign + Qiniu Download per item 9 v0.1.1 patch), `Form` (Upyun upload — M5 NEW validation), `Token` (Azure SAS + Qiniu Upload), `Headers` (still unused, available for future). Upyun FORM upload absorbed cleanly by the existing `DirectGrant.{Mode, URL, Method, Headers, FormFields, Token}` field set — no widening recommended. The `DirectGrantRequest.Extra map[string]string` escape hatch absorbed all 14 vendor-specific policy keys across qiniu (8 PutPolicy keys: callbackUrl, callbackBody, callbackHost, callbackBodyType, returnBody, returnUrl, saveKey, persistentOps) + upyun (6 policy keys: notify-url, apps, expiration-override, save-key, content-md5, allow-file-type) without growing the request struct. Cumulative v0.2.0 deferred candidates from M3+M4+M5 (none gating any milestone): (1) `MultipartService` sequential-only capability flag (gcs + qiniu both have it); (2) `MinPartSize` capability field (azure 4 MiB ≠ S3 5 MiB; promote to v0.2.0 if M6 surveys ≥1 more provider); (3) `credential.Provider.Rotate(ctx)` hook (still not needed in v1); (4) ~~Azure multipart `Initiate.Metadata` persistence to `Complete`~~ — RESOLVED in v0.1.1 (see item 9); (5) `DirectGrantRequest.Extra` formal documentation of vendor-recognised keys (qiniu + upyun); (6) `DirectGrant.ExtraReturned map[string]string` for vendor-side dispatch info return (qiniu + upyun); (7) Upyun SDK context-cancellation wiring via context-aware `http.Client`; (8) ~~`errors.As(&alreadyMapped)` early-return loses op/bucket/key context~~ — RESOLVED in v0.1.1 (see item 9). Plus the M3-phase-2 architect notes: `s3common.MapServiceCode`, `s3common.IsRetryableWithStatus`, `credential.MustExtractHMAC`, `s3common.S3FamilyCapabilities()` helper. **M6 (stabilization) is the next milestone**: benchmarks, examples, OTel semantic-conventions alignment, README quickstarts, then v1.0.0 cut.
9. **v0.1.1 architect-flagged correctness patch — DONE 2026-04-28**: 3 driver-internal items from the cumulative v0.2.0 list pulled forward to v0.1.1 because they were the only ones representing real correctness gaps (vs. capability-widening or doc/CI hardening). All 3 land WITHOUT touching `pkg/uos` frozen surface. (a) **Azure multipart `Initiate.Metadata` persistence**: `uploadSession` now carries `meta uos.Metadata` captured at `Initiate` and replayed at `CommitBlockList`; previously `metadata()` returned `nil` unconditionally and dropped caller-supplied metadata. (b) **`errors.As(&alreadyMapped)` cross-driver augmentation**: all 9 non-AWS drivers (`alibaba`, `azure`, `gcs`, `huawei`, `minio`, `qiniu`, `tencent`, `upyun`, `volcengine`) now augment the inner `*uos.Error` with caller's Provider/Operation/Bucket/Key context if those fields are empty, instead of identity-pass losing context. AWS uses a different `mapError` shape with no `errors.As(&alreadyMapped)` guard, so it was unaffected. (c) **Qiniu Download `Mode=Token` → `Mode=URL`**: `Signer.IssueDirectGrant(Operation=Download)` now returns `Mode=DirectGrantModeURL` (the architecturally honest encoding for Qiniu's URL-shaped Download Token); Qiniu's Mode=Token is now exclusively used for the Upload Token (true bearer-string-in-form-field). The frozen `DirectGrantModeToken` is still validated by Azure SAS + Qiniu Upload (still ≥2 providers per ADR rule). 5 of the original 8 v0.2.0 candidates remain deferred for M6/v0.2.0 evaluation.
