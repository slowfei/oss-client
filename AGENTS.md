...
4. `s3common` extraction — DONE pre-tag (lead refactor on 2026-04-28). Originally deferred to M3+; pulled forward when the AWS + MinIO duplication surface proved sufficient justification. `pkg/uos/s3common/` now hosts 5 shared helpers: `MapCodeString`, `MapHTTPStatus`, `MapContextErr`, `IsRetryable`, `LowerMetadataKeys`. Architect's full 5-candidate list reduced to 3 actually-duplicated pieces after hands-on inspection (pointer helpers and `formatRange` are AWS-only because minio-go uses native typed APIs); see `RELEASING.md` §5 Follow-up #4 for details.
