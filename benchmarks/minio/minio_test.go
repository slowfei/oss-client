...
// Package miniobench measures per-operation throughput for the MinIO driver
// (providers/minio) running against a testcontainers-spawned MinIO endpoint.
...
import (
	...
	miniodrv "github.com/maqian/object-storage-client/providers/minio"
)

// minioBenchEnv holds the shared state created once per benchmark binary
// invocation. It is populated by setupMinio and torn down via b.Cleanup.
type minioBenchEnv struct {
	client uos.Client
	bucket string
}

// setupMinio spins up a MinIO testcontainer and opens a MinIO-driver client
...
func setupMinio(b *testing.B) *minioBenchEnv {
	b.Helper()
	...
	if err != nil {
		b.Fatalf("setupMinio: SpawnMinIO: %v", err)
	}
	b.Cleanup(cleanup)
...
// BenchmarkSignURL_Read measures the pure CPU cost of presigned-URL generation
// (no network I/O). The URL is never fetched; this benchmark exercises only
// the minio-go signing path.
//
