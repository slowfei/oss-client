...
`github.com/slowfei/oss-client/providers/aws` — native driver for
...
| Field | Value |
| --- | --- |
| Module path | `github.com/slowfei/oss-client/providers/aws` |
...
```bash
go get github.com/slowfei/oss-client/providers/aws@providers/aws/v0.1.0
```
...
- `PathStyle bool` — forces path-style addressing (`bucket` in URL path instead of virtual-host). Leave `false` for virtual-host-compatible S3 endpoints; set `true` for MinIO and other targets that require path-style addressing.
